...
void CZlgcanDlg::OnBnClickedButtonSend()
{
    ...
    CString csText;
    csText.Format(_T("发送数量:%d, 成功数量:%d"), nSendCount, result);
	if (result != nSendCount)
	{
		AddData(_T("发送数据失败! ") + csText);
	}
	else
	{
		AddData(_T("发送数据成功 ") + csText);
	}
}
...
void CZlgcanDlg::AddData( const ZCAN_Transmit_Data* data, UINT len )
{
    ...
    for (UINT i = 0; i < len; ++i)
    {
        ...
        const canid_t& id = can.frame.can_id;
        sprintf_s(item, "Tx CAN ID:%08X %s %s 长度:%d 数据:", GET_ID(id), IS_EFF(id)?"扩展帧":"标准帧"
            , IS_RTR(id)?"远程帧":"数据帧", can.frame.can_dlc);
        for (UINT i = 0; i < can.frame.can_dlc; ++i)
        {
            size_t item_len = strlen(item);
...
void CZlgcanDlg::AddData( const ZCAN_TransmitFD_Data* data, UINT len )
{
    ...
    for (UINT i = 0; i < len; ++i)
    {
        ...
        const canid_t& id = can.frame.can_id;
        sprintf_s(item, "Tx CANFD ID:%08X %s %s 长度:%d 数据:", GET_ID(id), IS_EFF(id)?"扩展帧":"标准帧"
            , IS_RTR(id)?"远程帧":"数据帧", can.frame.len);
...
void CZlgcanDlg::OnBnClickedButtonGetTxAvailable()
{
    ...
    if (pRet)
    {
        ...
        csText.Format(_T("队列可用空间:%d"), nSpace);
    }
    else
    {
        csText.Format(_T("获取队列可用空间失败!"));
    }
    AddData(csText);
}
...
void CZlgcanDlg::OnBnClickedButtonClearTxQueue()
{
    ...
    CString csText;
    csText.Format(_T("清除队列发送 [%s]"), nRet > 0 ? _T("成功") : _T("失败") );
    AddData(csText);
}
...
void CZlgcanDlg::OnBnClickedCheckDelaySendMode()
{
    ...
    csText = (nDelaySendQueueMode ? _T("开启队列发送模式") : _T("关闭队列发送模式"));
    csRet.Format(_T(" [%s]"), nRet > 0 ? _T("成功") : _T("失败"));
...
void CZlgcanDlg::OnBnClickedButtonGetSendMode()
{
    ...
    {
        UINT nMode = *(int*)pRet;
        csText.Format(_T("设备当前模式:[%s]"), nMode ? _T("队列发送模式") : _T("正常发送模式"));
    }
    else
    {
        csText.Format(_T("获取通道模式失败!"));
    }
    AddData(csText);
}
...
void CZlgcanDlg::AddAutosendCan(int nEnable)
{
    ...
    CString csText;
    csText.Format(_T("添加CAN定时发送 索引:%d 启用:%d 周期:%d ms ID:0x%X [%s]"), autoObj.index, autoObj.enable, autoObj.interval, autoObj.obj.frame.can_id, (nRet ? _T("成功") : _T("失败")));
    AddData(csText);
}
...
void CZlgcanDlg::AddAutosendCanfd(int nEnable)
{
	...
	int nRet =  ZCAN_SetValue(device_handle_, path, (const char*)&autoObj);
    CString csText;
    csText.Format(_T("添加CANFD定时发送 索引:%d 启用:%d 周期:%d ms ID:0x%X [%s]"), autoObj.index, autoObj.enable, autoObj.interval, autoObj.obj.frame.can_id, (nRet ? _T("成功") : _T("失败")));
    AddData(csText);
}
...
void CZlgcanDlg::OnBnClickedButtonAutosendStart()
{
    ...
    CString csText;
    csText.Format(_T("开始定时发送 [%s]"), (nRet ? _T("成功") : _T("失败")));
    AddData(csText);
}
...
void CZlgcanDlg::OnBnClickedButtonAutosendStop()
{
	...
	int nRet = ZCAN_SetValue(device_handle_, path, "0");
    CString csText;
    csText.Format(_T("停止定时发送 [%s]"), (nRet ? _T("成功") : _T("失败")));
    AddData(csText);
}
...
void CZlgcanDlg::OnBnClickedButtonGetDevAutosend()
{
    ...
    if (pRet)
    {
        ...
        if (nCount > 0)
        {
            ...
            if (pRet)
            {
                ...
                {
                    csText.Format(_T("CAN 定时 索引:%d 间隔:%d ms ID:0x%08x"), pData[i].index, pData[i].interval, pData[i].obj.frame.can_id);
                    AddData(csText);
                }
            }
            else
            {
                csText.Format(_T("获取CAN定时发送数据失败!"));
                AddData(csText);
                return;
            }
        }
    ...
    {
        csText.Format(_T("获取CAN定时发送数量失败!"));
        AddData(csText);
        return;
    }
    ...
    if (pRet)
    {
        ...
        if (nCount > 0)
        {
            ...
            if (pRet)
            {
                ...
                for (UINT i = 0; i < nCount; i++)
                {
                    csText.Format(_T("CANFD 定时 索引:%d 间隔:%d ms ID:0x%08x"), pData[i].index, pData[i].interval, pData[i].obj.frame.can_id);
                    AddData(csText);
                }
            }
            else
            {
                csText.Format(_T("获取CANFD定时发送数据失败!"));
                AddData(csText);
                return;
            }
        }
    ...
    {
        csText.Format(_T("获取CANFD定时发送数量失败!"));
        AddData(csText);
        return;
    }
}
