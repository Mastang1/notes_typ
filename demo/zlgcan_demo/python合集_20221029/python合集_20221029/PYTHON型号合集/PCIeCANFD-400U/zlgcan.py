...
def canfd_start(zcanlib, device_handle, chn):
    ...
    ret = zcanlib.SetValue(ip,str(chn)+"/canfd_abit_baud_rate","250000")  #设置波特率
...
###SET filter  
    ...
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d PCIECANFD filter_clear failed!" %(chn))
        exit(0)
    ret = zcanlib.SetValue(ip,str(chn)+"/filter_mode","0")    #标准帧滤波
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d PCIECANFD filter_mode failed!" %(chn)) 
        exit(0)
    ret = zcanlib.SetValue(ip,str(chn)+"/filter_start","0")    
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d PCIECANFD filter_start failed!" %(chn))  
        exit(0)        
    ret = zcanlib.SetValue(ip,str(chn)+"/filter_end","0x7FF")    
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d PCIECANFD filter_end failed!" %(chn)) 
        exit(0)
    ...
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d PCIECANFD filter_ack failed!" %(chn))
        exit(0)
    ...
    ret = zcanlib.SetValue(ip,"0/set_device_merge","0")#0——分通道接收， 1——合并接收    
    if ret != ZCAN_STATUS_OK:
        print("Set set_device_merge failed!" %(chn))
        exit(0)

### Set Auto Transmit   
    ret = zcanlib.SetValue(ip,str(chn)+"/clear_auto_send","0")
    if ret != ZCAN_STATUS_OK:
        print("Clear CH%d PCIECANFD AutoSend failed!" %(chn))
        exit(0)
...
if __name__ == "__main__":
    ...
    handle = zcanlib.OpenDevice(ZCAN_PCIE_CANFD_400U_EX, 0,0)
