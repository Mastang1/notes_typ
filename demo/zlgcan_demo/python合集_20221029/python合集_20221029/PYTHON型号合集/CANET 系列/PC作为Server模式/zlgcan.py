...
def canet_start(zcanlib, device_handle):
    ...
    ret = zcanlib.SetValue(ip,  "0/work_mode", "1") #0-客户端   1-服务器  （注：当设备作为服务器模式时，PC要设置为客户端，反之亦然）
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET work_mode failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/local_port", "4001") #PC作为服务器模式下设置本地监听端口。
    if ret != ZCAN_STATUS_OK:
    ...
    if chn_handle == 0:
        print("initcan fail")
        exit(0)
    ret=zcanlib.StartCAN(chn_handle)==0           #启动“CAN通道”即为建立连接
    if ret ==0:
        print("startcan fail，可能当前TCP服务器端口被其它软件占用，请尝试换其它端口")
        exit(0)
    return chn_handle
...
if __name__ == "__main__":
    ...
    if handle == 0:
        print("Open CANET Device failed!请确保zlgcan.dll同级目录下，是否拷贝了同架构的kerneldlls文件夹")
        exit(0)
