...
def canet_start(zcanlib, device_handle):
    ...
    ret = zcanlib.SetValue(ip,  "0/work_mode", "1") 
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET work_mode failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/local_port", "8888") #UDP模式下设置本地监听端口。
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET local_port failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/ip", "192.168.0.178") #UDP模式下设置目标IP。
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET ip failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/work_port", "4001") #UDP模式下设置目标端口。
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET work_port failed!" )   
        exit(0)
...
if __name__ == "__main__":
    ...
    handle = zcanlib.OpenDevice(ZCAN_CANETUDP, 0,0)  
    if handle == INVALID_DEVICE_HANDLE:
        print("Open CANET Device failed!")
        exit(0)
