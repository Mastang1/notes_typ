...
def CANDTU_start(zcanlib, device_handle, chn):
    ...
    if ret != ZCAN_STATUS_OK:
        ...
        exit(0)
    ret = zcanlib.SetValue(ip, str(chn) + "/work_mode", "1") #1-正常，0-只听
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d CAN_E_U baud_rate failed!" %(chn))    
        exit(0)
    ret = zcanlib.SetValue(ip, str(chn) + "/acc_code", "0") #验收码为0
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d CAN_E_U baud_rate failed!" %(chn))  
        exit(0)
    ret = zcanlib.SetValue(ip, str(chn) + "/acc_mask", "0xFFFFFFFF") #屏蔽码全F
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d CAN_E_U baud_rate failed!" %(chn))  
        exit(0)
...
if __name__ == "__main__":
    ...
    if handle == INVALID_DEVICE_HANDLE:
        print("Open CANDTU-200UR device failed!")
        exit(0)
    print("Open CANDTU-200UR device success!")
    ...
    chn_handle = CANDTU_start(zcanlib, handle, 0)
