...
def canfd_start(zcanlib, device_handle, chn):
    ...
    ret = zcanlib.SetValue(ip, "0/tx_timeout","500")
    if ret == ZCAN_STATUS_OK:
        print("Set tx_timeout success!")
    else:print("Set tx_timeout fail!")
...
###SET filter  
    ...
    send_mode_flag =1 
    ret = zcanlib.SetValue(ip,"0/set_send_mode",str(send_mode_flag)) #设置队列发送模式：0-普通启动 ，1-队列启动，队列发送不可与定时发送共用
    if ret == ZCAN_STATUS_OK:
        print("set_queue_send_mode success!" )
    else: print("set queue fail")
...
if __name__ == "__main__":
    ...
    for i in range(transmit_num):
        ...
        msgs[i].frame.can_id  = 10
        msgs[i].frame.can_dlc = 8
        msgs[i].frame.__pad   = 0x80  #队列发送，当值为0x80时，单位是ms； 当值为0xC0时，单位是100us.
        msgs[i].frame.__res0  = 0x64   #帧间隔低位,定时100ms
        msgs[i].frame.__res1  = 0x00    #帧间隔高位
    ...
    for i in range(transmit_num):
        canfd_msgs[i].transmit_type = 0 #self test
        canfd_msgs[i].frame.eff     = 0 #extern frame
        canfd_msgs[i].frame.rtr     = 0 #remote frame
        canfd_msgs[i].frame.brs     = 1 #BRS 
        canfd_msgs[i].frame.can_id  = 0x100
        canfd_msgs[i].frame.len     = 8
        canfd_msgs[i].frame.brs     = 1
        canfd_msgs[i].frame.esi     = 0
        canfd_msgs[i].frame.__pad  = 0x20  #队列发送，当值为0x20时，单位为ms;  当值为0x30时，单位是100us。
        canfd_msgs[i].frame.__res0  = 0x64   #帧间隔低位,定时100ms
        canfd_msgs[i].frame.__res1  = 0x00    #帧间隔高位
