...
def canet_start(zcanlib, device_handle):
    ...
    if ret != ZCAN_STATUS_OK:
        ...
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/local_port", "8888") #PC作为服务器模式下设置本地监听端口。
    if ret != ZCAN_STATUS_OK:
...
if __name__ == "__main__":
    ...
    handle = zcanlib.OpenDevice(ZCAN_CANDTU_NET_400, 0,0)  
    if handle == INVALID_DEVICE_HANDLE:
        print("Open ZCAN_CANDTU_NET_400 Device failed!")
        exit(0)
