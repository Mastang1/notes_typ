...
CANET Start Demo
'''
def canet_start(zcanlib, device_handle):
    ip = zcanlib.GetIProperty(device_handle)
    ret = zcanlib.SetValue(ip,  "0/work_mode", "0") #0-客户端   1-服务器  （注：当设备作为服务器模式时，PC要设置为客户端，反之亦然）
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET work_mode failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/ip", "192.168.0.178") #ip,PC作为客户端模式时需要设置目标IP，也就是填CANET的IP
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET ip failed!" )
        exit(0)
    ret = zcanlib.SetValue(ip,  "0/work_port", "4001") #PC作为客户端模式时需要设置目标端口号，也就是填CANET的CAN口工作端口号
    if ret != ZCAN_STATUS_OK:
        print("Set  CANET wort_port failed!" )
        exit(0)
    zcanlib.ReleaseIProperty(ip) 
    chn_init_cfg = ZCAN_CHANNEL_INIT_CONFIG()
    chn_handle = zcanlib.InitCAN(device_handle, 0, chn_init_cfg)   # chn_init_cfg结构体以及inican不做实际操作，仅用于获取“通道句柄”chn_handle
    if chn_handle ==0:
        print("initCAN failed!" %(chn))  
        exit(0)
    ret=zcanlib.StartCAN(chn_handle)           #启动“CAN通道”即为建立连接
    if ret ==0 :
        print("StartCAN fail ,连接设备失败，请确保目标IP和目标端口是否填写正确，并确保PC与以太网CAN卡处于同一网段")
        exit(0)
...
if __name__ == "__main__":
    zcanlib = ZCAN() 
    handle = zcanlib.OpenDevice(ZCAN_CANFDNET_200U_TCP, 0,0)  
    if handle == INVALID_DEVICE_HANDLE:
        print("打开设备失败! 请确保zlgcan.dll同级目录下拷贝了同架构的kerneldlls文件夹")
        exit(0)
    print("device handle:%d." %(handle))

    #Start CAN
    chn_handle = canet_start(zcanlib, handle)
    ...
    for i in range(transmit_num):
        msgs[i].transmit_type = 2 #self test
    ...
    for i in range(transmit_num):
        canfd_msgs[i].transmit_type = 2 #self test
